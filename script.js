...
(function () {
    ...
    window.alert = function (msg) {
        ...
        var type = (msg.includes('Ошибка') || msg.includes('Error') || msg.includes('❌')) ? 'error'
...
(function () {
    ...
    function applyFilter() {
        ...
        // Скрываем секции где все карточки спрятаны
    ...
    // Сбрасываем фильтр при переходе в раздел игр
    var _origShowGames = window.showGames;
    window.showGames = function () {
        if (_origShowGames) _origShowGames();
        activeGenre = 'all';
        document.querySelectorAll('.genre-btn').forEach(function (b) { b.classList.remove('active'); });
        var allBtn = document.querySelector('.genre-btn[data-genre="all"]');
        if (allBtn) allBtn.classList.add('active');
        setTimeout(applyFilter, 50);
    };
}());

// ===== ТРИГГЕРЫ ГЕЙМИФИКАЦИИ =====
(function() {

    // Трекаем клик по карточке игры
    document.addEventListener('click', function(e) {
        var card = e.target.closest && e.target.closest('.card[data-game]');
        if (card) {
            var id = card.getAttribute('data-game');
            if (id && window.gpTrackGame) window.gpTrackGame(id);
        }
    }, true);

    // Трекаем фильтр по жанру
    document.querySelectorAll('.genre-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
    ...
    if (si) {
        ...
        si.addEventListener('input', function() {
            if (!searched && si.value.length > 1) {
                searched = true;
                if (window.gpTrackAction) window.gpTrackAction('search');
            }
        });
    }

    // Трекаем сохранение ПК
    var saveBtn = document.getElementById('savePcBtn');
    if (saveBtn) {
        saveBtn.addEventListener('click', function() {
            setTimeout(function() {
                if (window.gpTrackAction) window.gpTrackAction('set_pc');
            }, 200);
        });
    }

    // Трекаем смену темы
    document.querySelectorAll('[onclick*="setTheme"], [data-theme]').forEach(function(btn) {
        btn.addEventListener('click', function() {
            if (window.gpTrackAction) window.gpTrackAction('theme');
        });
    });

    // Трекаем случайную игру
    var randBtn = document.getElementById('navRandom');
    if (randBtn) {
        randBtn.addEventListener('click', function() {
            if (window.gpTrackAction) window.gpTrackAction('random');
        });
    }

    // Рендерим достижения когда открывается профиль
    var _origShowProfile = window.showProfile;
    window.showProfile = async function() {
        if (_origShowProfile) await _origShowProfile();
        if (window.gpRenderAchievements) window.gpRenderAchievements();
        // обновляем XP UI
        var xpEl = document.getElementById('profileXPText');
        if (xpEl && window.gpUpdateLevelUI) window.gpUpdateLevelUI();
    };

    // Трекаем избранное — патчим счётчик
    var favCountEl = document.getElementById('profileFavoritesCount');
    if (favCountEl) {
        var observer = new MutationObserver(function() {
            var n = parseInt(favCountEl.textContent) || 0;
            if (window.gpTrackFavs) window.gpTrackFavs(n);
        });
        observer.observe(favCountEl, { childList: true, characterData: true, subtree: true });
    }
